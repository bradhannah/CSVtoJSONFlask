...
class CsvToJson:
    ...
    def _csv_lines_to_dict(self, csv_lines):
        ...
        i = 0
        header_list = None  #
        csv_dissected_rows = csv.reader(csv_lines, delimiter=',', skipinitialspace=True)
                                        #, strict=True)
    ...
    def keyed_csv_to_array(self):
        if type(self._raw_csv_lines) != list:
            return "FAILED"
        csv_dict = self._csv_lines_to_dict(self._raw_csv_lines)
